...
function auto_preset($Field, $Table, $Config = false){
	$Pre = '';
	if(!empty($Config['_AutoValue'][$Field])){
		$Pre = $Config['_AutoValue'][$Field];
	}
	$Return = 'Session Value: <input type="text" name="data[_AutoValue]['.$Field.']" value="'.$Pre.'" class="textfield" size="8" /> ';
return $Return;
}
function auto_get($Field, $Table, $Config = false){
	$Pre = '';
	if(!empty($Config['_AutoGet'][$Field])){
		$Pre = $Config['_AutoGet'][$Field];
	}
	$Return = 'Get Value: <input type="text" name="data[_AutoGet]['.$Field.']" value="'.$Pre.'" class="textfield" size="8" /> ';
return $Return;
}
function session_config($Field, $Table, $Config = false){
	...
	$filterCap = '';
	if(!empty($Config['_UserBaseFilterCapture'][$Field])){
		$filterCap = 'checked="checked"';
	}
	$Return = 'Capture Updates: <input type="checkbox" name="data[_UserBaseFilterCapture]['.$Field.']" value="1" '.$filterCap.' />';
	$Return .= '<br />&nbsp;Apply Hard Filter: <input type="checkbox" name="data[_UserBaseFilter]['.$Field.']" value="1" '.$filter.' />';
...
function session_value($Field, $Table, $Config = false){
	$Pre = '';
	if(!empty($Config['_SessionValue'][$Field])){
		$Pre = $Config['_SessionValue'][$Field];
	}
	$Return = 'Session Value: <input type="text" name="data[_SessionValue]['.$Field.']" value="'.$Pre.'" class="textfield" size="8" /> ';
return $Return;
}
