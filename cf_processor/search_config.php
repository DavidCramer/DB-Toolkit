...
{{#if tables}}
	...
	<div id="dbt-field-config-{{_id}}">
		...
		<div class="caldera-config-group">
				...
				<div class="caldera-config-field">
					<select class="block-input caldera-field-bind" id="{{../_id}}_fields_{{@key}}" name="{{../_name}}[fields][{{@key}}]" data-default="{{this}}"></select>
				</div>
		</div>
		{{/each}}
...
<div class="caldera-config-group">
	...
	<div class="caldera-config-field">
		{{{_field slug="location" exclude="system" type="hb_template"}}}
	</div>
</div>
